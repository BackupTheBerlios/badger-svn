...
class BackgroundImagePDF {
  ...
  function is_default() { return is_null($this->url); }
  ...
  function show(&$viewport, $box, $repeat, $position) {
    ...
    if ($position->x_percentage) {
      $x_offset = ($padding_width  - $image_width*px2pt(1))  * $position->x / 100;
    } else {
      $x_offset = $position->x;
    }

    if ($position->y_percentage) {
      $y_offset = ($padding_height - $image_height*px2pt(1)) * $position->y / 100;
    } else {
      $y_offset = $position->y;
    };
  ...
  function to_ps_init(&$psdata) {
    ...
    $id = $image_encoder->auto($psdata, $image_res, $size_x, $size_y, $tcolor, $image, $mask);
    $init = "image-".$id."-init";
