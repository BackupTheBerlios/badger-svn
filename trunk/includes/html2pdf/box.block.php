<?php
// $Header: /cvsroot/html2ps/box.block.php,v 1.40 2005/12/24 15:37:56 Konstantin Exp $
...
class BlockBox extends GenericContainerBox {
  ...
  function get_block_width(&$context) {
    if (!is_a($this->_width_constraint, "WCNone") || 
        $this->float == FLOAT_NONE) {
      return $this->get_width();
    } else {
      return $this->get_max_width_natural($context);
    };
  }

  function _get_hor_extra() {
    return GenericBox::_get_hor_extra();
  }
  ...
  // Note: it is non-obvios from the CSS standard that of neither top nor bottom property is specified
  function reflow_absolute(&$context) {
    ...
    // Calculate horizontal positions of the block; 
    ...
    // As absolute-positioned box generated new flow contenx, extend the height to fit all floats
    $float_bottom = $context->float_bottom();     
    if (!is_null($float_bottom)) {
      $this->extend_height($float_bottom);
    };

    // If element have been positioned using 
    if (is_null($this->left) && !is_null($this->right)) {
  ...
  function show_fixed(&$viewport) {
    $this->moveto($viewport->left + $this->get_css_left_value(),
                  $viewport->bottom + $viewport->height + $viewport->offset - $this->top[0]);

    // draw generic box
    return GenericContainerBox::show_fixed($viewport);
  }
