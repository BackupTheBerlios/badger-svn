...
if(!class_exists('FPDF'))
{
  ...
  class PDFAppearanceStream extends PDFIndirectObject {
    function PDFAppearanceStream($object_id, $generation_id) {
      $this->PDFIndirectObject($object_id, $generation_id);
    }

    function pdf(&$handler) {
      $stream_content  = "0.753 g 0 0 109 16 re f ";
      $dict_content   = array(
                              'Type'     => "/XObject",
                              'Subtype'  => "/Form",
                              'FormType' => "1",
      ...
      $content .= "\n";
      $content .= $handler->_stream($stream_content);
  ...
  class PDFAnnotationWidget extends PDFAnnotation {
    ...
    function PDFAnnotationWidget($object_id, $generation_id, &$parent) {
  ...
  class PDFFieldText extends PDFIndirectObject {
    var $annotation;

    function PDFFieldText($object_id, $generation_id) {
      $this->PDFIndirectObject($object_id, $generation_id);
    }

    function &create(&$handler, $generation_id=0) {
      $field = new PDFFieldText($handler->_generate_new_object_number(), $generation_id);
      $field->annotation = new PDFAnnotationWidget($handler->_generate_new_object_number(), $generation_id, $field);
      $field->annotation->appearance = new PDFAppearanceStream($handler->_generate_new_object_number(), $generation_id);
      return $field;
    }
    ...
    function pdf(&$handler) {
      $content = $handler->_dictionary(array(
                                             ...
                                             'DV'   => "(The quick brown fox ate the lazy mouse)",
                                             'Kids' => $handler->_reference_array(array($this->annotation))
                                             ));
      return $content;
    }
  }
  ...
  class FPDF {
    ...
    function add_form_field() {
      ...
      $this->_pages[count($this->_pages)-1]->add_annotation($form->annotation);
    ...
    function Circle($x, $y, $r) {
      ...
      $this->_out(sprintf("%.2f %.f2 %.2f %.2f %.2f %.2f c", 
                          ...
                          $x - $l, $y - $r,
                          $x, $y - $r));      
      $this->_out(sprintf("%.2f %.f2 %.2f %.2f %.2f %.2f c S", 
                          $x + $l, $y - $r,
                          $x + $r, $y - $l, 
                          $x + $r, $y));      
    }
    ...
    function _enddoc() {
      ...
      for ($i=0; $i<count($this->_form_fields); $i++) {
        ...
        $this->offsets[$form->annotation->appearance->get_object_id()] = strlen($this->buffer);
        $this->_out($this->_indirect_object($form->annotation->appearance));

        $this->offsets[$form->annotation->get_object_id()] = strlen($this->buffer);
        $this->_out($this->_indirect_object($form->annotation));
